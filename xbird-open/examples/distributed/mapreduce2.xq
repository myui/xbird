...
declare variable $length := 1000;

declare function local:select1() {
	execute at $remote-endpoint1 { 
		fn:collection($colname)/site/closed_auctions/closed_auction[price/text() >= 300]
	}
};

declare function local:select2() {
	execute at $remote-endpoint2 { 
		fn:collection($colname)/site/open_auctions/open_auction[initial/text() >= 300]
	}
};

declare function local:reduce() {
	execute at $remote-endpoint3 {
		(fn:subsequence(local:select1(), 1, $length) | fn:subsequence(local:select2(), 1, $length))
	}
};
