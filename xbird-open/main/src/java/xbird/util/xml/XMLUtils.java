 ...
 * @link http://www-128.ibm.com/developerworks/xml/library/x-wxxm30.html
 * @link XMLChar
 */
public final class XMLUtils {
            ...
            if(c == '&') {
                ...
                if(pos == -1) {
                    ...
                    if(substr.equals("&amp;")) {
                        ...
                        throw new IllegalStateException("Invalid XML charactor `" + substr
                                + "` is detected at " + i + " of " + str + '!');
                    }
                    i = pos;
                }
            } else {
    ...
    public static String normalizeString(String src) {
        int length;
        if(src == null || (length = src.length()) == 0 || length < 2) {
            return src;
        }
        CharSequence cs;
        if(Character.isWhitespace(src.charAt(0))) {
            cs = StringUtils.stripStart(src.substring(1), null); // retain first space
        } else {
            cs = src;
        }
        final int last = cs.length() - 1;
        if(last > 2 && Character.isWhitespace(cs.charAt(last))) {
            cs = StringUtils.stripEnd(cs.subSequence(0, last - 1), null); // retain last space           
        }
        return cs.toString();
    }

    private static final String URICHAR = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_.~";
    private static final String URICHAR_RESERVED = "!*'();:@&=+$,/?%#[]"; // excluce '%'
     ...
     * @param escapeReserved true to escape IRI, otherwise false to escape URI
     */
    public static String escapeUri(final String src, final boolean escapeReserved) {
        assert (src != null);
        final int srclen = src.length();
        final StringBuilder buf = new StringBuilder(srclen);
        for(int i = 0; i < srclen; i++) {
            final char c = src.charAt(i);
            if(URICHAR.indexOf(c) >= 0) {
                buf.append(c); // escape is not required
            } else if(!escapeReserved && URICHAR_RESERVED.indexOf(c) >= 0) {
                buf.append(c);
            } else {
                // Each such character is replaced in the string by an escape sequence, 
                // which is formed by encoding the character as a sequence of octets in UTF-8, 
                // and then representing each of these octets in the form %HH, 
                // where HH is the hexadecimal representation of the octet.
                escapeChar(c, (i + 1 < srclen) ? src.charAt(i + 1) : ' ', buf);
            }
        }
        return buf.toString();
    }
     ...
     */
    public static String escapeHtmlUri(String src) {
        final int srclen = src.length();
        StringBuilder escaped = null;
        for(int i = 0; i < srclen; i++) {
            final char c = src.charAt(i);
            if(c < 32 || c > 126) { // have to escape
                if(escaped == null) {
                    escaped = new StringBuilder(srclen);
                    if(i > 1) {
                        escaped.append(src.subSequence(0, i - 1));
                    }
                    src = Normalizer.normalize(src, Normalizer.NFC);
                }
                escapeChar(c, (i + 1 < srclen) ? src.charAt(i + 1) : ' ', escaped);
            } else if(escaped != null) {
                escaped.append(c);
            }
        }
        return escaped == null ? src : escaped.toString();
    }

    private static void escapeChar(final char c1, final char c2, final StringBuilder buf) {
        final byte[] b = UTF8Codec.encode(new int[] { c1, c2 });
        final int size = b.length;
        assert ((size & 1) == 0) : size;
        for(int i = 0; i < size; i += 2) {
            buf.append('%');
            final int high = ((b[i] & 0xF0) >> 4);
            buf.append(HEXSYMBOL[high]);
            final int low = (b[i] & 0x0F);
            buf.append(HEXSYMBOL[low]);
        }
    }
    ...
    /**
     * Is valid xml charactor?
     */
    public static boolean isValid(int c) {
        return (c < 0x10000 && (CHARS[c] & MASK_VALID) != 0) || (0x10000 <= c && c <= 0x10FFFF);
    }

    public static void normalizeAndPrint(Writer writer, String s, boolean isAttValue) {
        final int len = (s != null) ? s.length() : 0;
        for(int i = 0; i < len; i++) {
            final char c = s.charAt(i);
            normalizeAndPrint(writer, c, isAttValue);
        }
    }

    public static void normalizeAndPrint(Writer writer, char c, boolean isAttValue) {
        switch(c) {
            case '<':
                print(writer, "&lt;");
                break;
