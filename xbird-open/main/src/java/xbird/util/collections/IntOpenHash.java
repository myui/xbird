 ...
 */
public class IntOpenHash<V> implements Externalizable {
    ...
    protected IntOpenHash(int size, float loadFactor, float growFactor, boolean forcePrime) {
        if(size < 1) {
    ...
    public IntOpenHash(int size, float loadFactor, float growFactor) {
        this(size, loadFactor, growFactor, true);
    }

    public IntOpenHash(int size) {
        this(size, DEFAULT_LOAD_FACTOR, DEFAULT_GROW_FACTOR, true);
    }

    public IntOpenHash() {// required for serialization
    ...
    public static final class IntOpenLRUMap<V> extends IntOpenHash<V> {
