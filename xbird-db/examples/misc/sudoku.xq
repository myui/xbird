...
declare function local:getRow ($board as xs:integer+, $index as xs:integer) as xs:integer+ {
...
$index), local:getCol($board, $index), local:getGroup($board, $index))
return
   ...
   return
    if (empty($result)) then
     if (count($possibleValues) > 1) then
       local:tryValues($board, $emptyCells, $possibleValues[position() != 1])
     else
       ()
   else
    $result
...
declare  function  local:populateValues($board as xs:integer+,
 ...
 if (not(empty($emptyCells)))
  ...
  then
      local:tryValues($board, $emptyCells, $possibleValues)
  else if (count($possibleValues) = 1)
      then
 let $newBoard2 as xs:integer+ :=($board[position() <
...
local:drawResult(local:solveSudoku($board))(: Stylus Studio meta-information - (c) 2004-2005. Progress Software Corporation. All rights reserved.
<metaInformation>
