...
for $a in fn:distinct-values(
		for $b in fn:doc("bib.xml")/bib/book, $t in $b/title
...
(:
	XQuery UseCase 1.1.9.4 Q4
	http://www.w3.org/TR/2002/WD-xmlquery-use-cases-20021115/#xmp-queries-results-q4
:)

declare variable $bibref := "file:///D:/workspace/xbird/main/test/resources/org/metabrick/xbird/xquery/parser/data/bib.xml";

let $v :=
  <results>
    {
      for $a in fn:distinct-values(fn:doc($bibref)//author)
      return
          <result>
              { $a }
              {
                  for $b in fn:doc($bibref)/bib/book
                  where some $ba in $b/author satisfies fn:deep-equal($ba,$a)
                  return $b/title
              }
          </result>
    }
